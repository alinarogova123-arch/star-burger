...
Пример рабочего сайта можно увидеть по ссылке http://kotarba.ru/
...
### 4. Создать суперпользователя Django можно следующей командой:
...
### 1. Скачайте строго `master` ветку репозитория на свой сервер, чтобы избежать конфликтов:
```bash
git clone --branch master --single-branch git@github.com:alinarogova123-arch/star-burger.git
```

### 2. Перейдите в директорию вашего проекта и создайте файл .env со следующими переменными:
```
...
DB_URL=postgresql://db_user_name:Пароль_пользователя_базы_данных@db:5432/db_name
...
### 4. Включите созданную конфигурацию:
```bash
...
### 7. Установите следующие права доступа для nginx:
```
...
### 8. Переходите на ваш сайт по домену или IP_адресу.

### Ошибка `signal: killed` на этапе `npm ci`
Если сервер принудительно завершает сборку фронтенда, это означает нехватку оперативной памяти (RAM). Создайте файл подкачки (Swap) на 4 ГБ:
```bash
...
### Конфликт миграций в Docker (`Conflicting migrations detected`)
Если скрипт деплоя упал на этапе миграций, выполните слияние и сборку вручную:
```bash
